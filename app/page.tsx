...
export default function Home() {
	...
	const latestPosts = sortedPosts.slice(0, 3).map((post, index) => {
		const isNew = sortedPosts.length === 1 || index === 0;
		return { ...post, isNew };
	});
		...
		<>
			<section className="space-y-6 pb-8 pt-6 md:pb-12 md:mt-8 lg:py-32">
				<div className="container flex flex-col gap-4 text-center">
					<h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-balance">
						Welcome to{" "}
						<span className="text-accent underline  underline-offset-4 italic">
							KAptured
						</span>
					</h1>
          ...
          What sparks your curiosity today? Step into my space, where ideas and reflections await to nourish your mind.
					...
					<p className="max-w-[42rem] mx-auto sm:text-xl text-balance mt-6 text-text">
						...
						reflections. Step into my space where fleeting moments transform
						into musings and ideas take shape. Let’s explore these sparks
						together.
					</p>
